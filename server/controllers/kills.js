...
const { findAndSort, findOne, findAndCount, aggregate, aggregateTop } = require('../service/DB')
const { getCurrentEventId } = require('../event')
module.exports = async (req, res, next) => {
  try {
    const reqParams = req.params
    let eventId = reqParams.eventId !== '0' ? reqParams.eventId : await getCurrentEventId()
